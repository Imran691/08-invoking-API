//this page will render data dynamically, i.e. every time a user visits the page, API will be called, API will fetch data from the server and will render this data.
...
export default async function DynamicPage  ()  {
  ...
  return (
    <div>
      ...
      {books.map( (book:Book) => (
        <ul>
          <li key={book.id}>{book.name} - {book.type} - {book.available}</li>
        </ul>
      ))}
    </div>
  )
